$(document).ready(function () {
    ...
    if ($('#editor').length > 0) {
        CKEDITOR.replace('editor', {
            ...
            filebrowserImageUploadUrl: route_prefix + '/upload?type=Images&_token=' + $('meta[name="csrf-token"]').attr('content'),
            filebrowserBrowseUrl: route_prefix + '?type=Files',
            filebrowserUploadUrl: route_prefix + '/upload?type=Files&_token=' + $('meta[name="csrf-token"]').attr('content')

        });
    }

    // marquee string
    $('.marquee').marquee({ duplicated: true, duration: 10000, startVisible: true }).mouseover(function () {
        $(this).trigger('pause');
    }).mouseout(function () {
        $(this).trigger('resume');
    ...
    $('[data-switcher]').change(function () {
        ...
        let value = $(this).val();
        $('[data-' + field + ']').prop('disabled', true).attr('disabled', 'disabled').removeClass('d-none').addClass('d-none');
        $('[data-' + field + '="' + value + '"]').prop('disabled', false).removeAttr('disabled').removeClass('d-none');
    });
    ...
    $('[data-param]').change(function () {
        if (!!$(this).val())
            $(this).attr('name', $(this).data('param'));
        else
    ...
    $('#confirmModal').on('show.bs.modal', function (event) {
        ...
        modal.find('#confirm').off().click(function () {
            window.eval(confirm);
            modal.modal('hide');
        });
    });
    ...
    window.addEventListener('load', function () {
        ...
        var validation = Array.prototype.filter.call(forms, function (form) {
            ...
            form.addEventListener('reset', function (event) {
                $(form).find('input:visible, select:visible').val('').trigger('change');
                event.preventDefault();
                event.stopPropagation();
            }, false);
        });
...
$(function () {
    ...
    function SidebarCollapse(elem) {
        ...
        if (window.innerWidth >= 992) {
            $('#sidebar:not(.collapsed) [data-tooltip="collapsed"]').tooltip('dispose');
            $('#sidebar.collapsed [data-tooltip="collapsed"]').tooltip({ boundary: 'window' });
        }
    }
});
    ...
    window.addEventListener('DOMContentLoaded', function (e) {
        ...
        for (var i in preloaders || []) {
            if (preloaders[i].dataset) {
                var target = document.querySelector(preloaders[i].dataset.target) || {};
...
$(function () {
    $('#dropdown-filter .right-carets').on('click', function (e) {
        ...
        if (!$(this).parent().next().hasClass('show')) {
            $(this).parent().parents('#dropdown-filter .dropdown-menu').first().find('.show').removeClass('show');
        }
        var $subMenu = $(this).parent().next('#dropdown-filter .dropdown-menu');
        ...
        $(this)
            ...
            .each(function (index, rightCaret) {
                $(rightCaret).find('.dropdown-rotate-caret').removeClass('dropdown-rotate-caret');
            });
        $(this).find('.fa-caret-right').toggleClass('dropdown-rotate-caret');


        $(this).parent().parents('#dropdown-filter li.nav-item.dropdown.show').on('hidden.bs.dropdown', function (e) {
            ...
            $('#dropdown-filter .dropdown-submenu .show').removeClass('show');
        });
        return false;
    });
    $('#dropdown-filter .filter-event').click(function () {
        ...
        $('#dropdown-btn-text').next().remove();
        let arrayFilterValues = $(this).data('values');
        let formFilter = $('.for-dropdown-filter').first();
        formFilter.append(`<input type="hidden" name="type" value="${arrayFilterValues[0]}"/>`);
        if (arrayFilterValues[1] !== undefined) formFilter.append(`<input type="hidden" name="category" value="${arrayFilterValues[1]}"/>`);
        if (arrayFilterValues[2] !== undefined) formFilter.append(`<input type="hidden" name="kind" value="${arrayFilterValues[2]}"/>`);
        startPreloader();
        formFilter.submit();
    });
    $('#filter-clear').click(function () {
        ...
        $(this).remove();
        startPreloader();
        $('.for-dropdown-filter').first().submit();
    });
});
...
$(document).ready(function () {
    ...
    ).setMaxBounds(L.latLngBounds(L.latLng(44.00, 68.00), L.latLng(39.00, 81.00)));
    ...
    $.get('/js/analysis-data.json', function (data) {
        ...
        data.forEach(item => {
            ...
            var markerPopup = '';
            var coordinates = item['Широта'] && item['Долгота'] ? [item['Широта'], item['Долгота']] : [];
            ...
            var gumusValue = parseFloat(item[gumusColumn] ? item[gumusColumn] : 0);
            var phValue = parseFloat(item[phColumn] ? item[phColumn] : 0);
            var azotValue = parseFloat(item[azotColumn] ? item[azotColumn] : 0);
            var fosforValue = parseFloat(item[fosforColumn] ? item[fosforColumn] : 0);
            var kaliyValue = parseFloat(item[kaliyColumn] ? item[kaliyColumn] : 0);
            var natriyValue = parseFloat(item[natriyColumn] ? item[natriyColumn] : 0);
        ...
        coords.forEach(item => {
            ...
            if (item.items.length > 0) {
                ...
                item.items.forEach(subItem => {
                    gumusValue += parseFloat(subItem[gumusColumn] ? subItem[gumusColumn] : 0);
                    phValue += parseFloat(subItem[phColumn] ? subItem[phColumn] : 0);
                    azotValue += parseFloat(subItem[azotColumn] ? subItem[azotColumn] : 0);
                    fosforValue += parseFloat(subItem[fosforColumn] ? subItem[fosforColumn] : 0);
                    kaliyValue += parseFloat(subItem[kaliyColumn] ? subItem[kaliyColumn] : 0);
                    natriyValue += parseFloat(subItem[natriyColumn] ? subItem[natriyColumn] : 0);
                });
